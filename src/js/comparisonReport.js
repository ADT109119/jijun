...
export class ComparisonReport {
    ...
    async calculateComparison(periodType, periods, options = {}) {
        ...
        categoryComparisons.sort((a, b) => {
            ...
            const sumA = periods.reduce(
                (s, _, i) => s + Math.abs(a[`period${i}`] || 0),
                0
            )
            const sumB = periods.reduce(
                (s, _, i) => s + Math.abs(b[`period${i}`] || 0),
                0
            )
            return sumB - sumA
        })
    ...
    renderSummaryCards(container, periodData) {
        ...
        for (const pd of periodData) {
            ...
            const prevData =
                currentIndex > 0 ? periodData[currentIndex - 1] : null
            let changeBadge = ''
            if (prevData) {
                const prevNet = prevData.income - prevData.expense
                if (prevNet !== 0) {
                    const change = (
                        ((net - prevNet) / Math.abs(prevNet)) *
                        100
                    ).toFixed(1)
                    const sign = change >= 0 ? '+' : ''
                    const cls =
                        change >= 0 ? 'text-wabi-income' : 'text-wabi-expense'
    ...
    renderCategoryTable(container, periodLabels, categoryComparisons) {
        ...
        for (const row of categoryComparisons) {
            ...
            // Category name
            const catObj =
                this.categoryManager.getCategoryById('expense', row.category) ||
    ...
    static calculateDailyAverages(periodData, periodType, periodLabels) {
        return periodData.map((pd, i) => {
            const days = ComparisonReport.getDaysInPeriod(
                periodType,
                periodLabels[i]
            )
    ...
    renderCategoryRankings(
        ...
        for (let i = 0; i < periodLabels.length; i++) {
            ...
            if (rankings[i].length === 0) {
                html +=
                    '<p class="text-xs text-wabi-text-secondary py-2">無支出分類</p>'
            } else {

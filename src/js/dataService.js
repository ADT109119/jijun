...
class DataService {
            ...
            if (oldVersion < 12) {
              if (db.objectStoreNames.contains('records')) {
                ...
                if (!recordStore.indexNames.contains('amortizationId')) {
                  recordStore.createIndex('amortizationId', 'amortizationId', {
                    unique: false,
                  })
                }
              }
  ...
  async addRecurringTransaction(transaction, skipLog = false) {
    try {
      ...
      await tx.done
      if (!skipLog)
        await this.logChange('add', 'recurring_transactions', id, dataToSave)
      return id
    } catch (error) {
  ...
  async deleteRecurringTransaction(id, skipLog = false) {
    try {
      ...
      await tx.done
      if (!skipLog)
        await this.logChange('delete', 'recurring_transactions', id, { uuid })
      return true
    } catch (error) {
  ...
  async exportData(options = {}) {
    ...
    try {
      ...
      allSettings.forEach(s => {
        if (
          s.key &&
          (s.key === 'budget_settings' || s.key.startsWith('budget_settings_'))
        ) {
        ...
        try {
          ...
          if (
            ...
            for (const ledger of data.ledgers) {
              ...
              const { id: _unusedId, ...ledgerData } = ledger
          ...
          const _importPerLedgerCategories = async (dataVal, settingKey) => {
            ...
            const isLegacyFormat =
              dataVal.expense !== undefined || dataVal.income !== undefined
            if (isLegacyFormat) {
          ...
          if (
            ...
            for (const account of data.accounts) {
              ...
              const { id: _unusedId, ...accountData } = account
          ...
          if (data.contacts && Array.isArray(data.contacts)) {
            for (const contact of data.contacts) {
              ...
              const { id: _unusedId, ...contactData } = contact
          ...
          if (data.debts && Array.isArray(data.debts)) {
            ...
            for (const debt of data.debts) {
              const oldId = debt.id
              const { id: _unusedId, ...debtData } = debt
              ...
              if (
                ...
                debtData.remainingAmount =
                  debtData.originalAmount ?? debtData.amount ?? 0
              }
              if (
                ...
                debtData.originalAmount =
                  debtData.amount ?? debtData.remainingAmount ?? 0
              }
          ...
          for (const record of validRecords) {
            ...
            const { id: _unusedId, ...recordData } = record
            ...
            try {
              ...
              for (const am of data.amortizations) {
                ...
                if (
                  ...
                  amData.accountId = oldAccountIdToNewIdMap.get(
                    amData.accountId
                  )
                }
  ...
  async exportDataForSync(options = {}) {
    ...
    const rawRecords = (await this.getRecords({ allLedgers: true })).filter(r =>
      validLedgerIds.has(r.ledgerId)
    )
    const rawAccounts = (await this.getAccounts({ allLedgers: true })).filter(
      a => validLedgerIds.has(a.ledgerId)
    )
    const rawContacts = (await this.getContacts({ allLedgers: true })).filter(
      c => validLedgerIds.has(c.ledgerId)
    )
  ...
  async addAccount(account, skipLog = false) {
    try {
      ...
      // 同步接收路徑：移除來源裝置的 integer id，讓 IndexedDB 自動產生新 id
      const { id: _unusedId, ...accountWithoutId } = account
      ...
      await tx.done
      if (!skipLog)
        await this.logChange('add', 'accounts', id, { ...dataToAdd, id })
      return id
    } catch (error) {
  ...
  async addDebt(debt, skipLog = false) {
    try {
      ...
      if (skipLog) {
        ...
        const amount =
          debt.amount ?? debt.originalAmount ?? debt.remainingAmount ?? 0
        debtData = {
          ...debt,
          contactUuid,

...
class DataService {
        ...
        try {
          ...
          if (
            ...
            for (const account of data.accounts) {
              ...
              const { id: _id, ...accountData } = account // eslint-disable-line no-unused-vars
          ...
          if (data.contacts && Array.isArray(data.contacts)) {
            for (const contact of data.contacts) {
              ...
              const { id: _id, ...contactData } = contact // eslint-disable-line no-unused-vars
          ...
          if (data.debts && Array.isArray(data.debts)) {
            ...
            for (const debt of data.debts) {
              const oldId = debt.id
              const { id: _id, ...debtData } = debt // eslint-disable-line no-unused-vars
          ...
          for (const record of validRecords) {
            ...
            const { id: _id, ...recordData } = record // eslint-disable-line no-unused-vars
  ...
  async addAccount(account, skipLog = false) {
    try {
      ...
      // 同步接收路徑：移除來源裝置的 integer id，讓 IndexedDB 自動產生新 id
      const { id: _rid, ...accountWithoutId } = account // eslint-disable-line no-unused-vars

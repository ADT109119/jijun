...
export class RewardService {
  ...
  async renderBannerAd(container) {
    ...
    if (isNative && this._admobModule) {
      try {
        const { BannerAdSize, BannerAdPosition } =
  ...
  async _showNativeRewardedAd() {
    ...
    return new Promise(resolve => {
      ...
      // 監聽：用戶獲得獎勵
      rewardHandle = AdMob.addListener('onRewardedVideoAdReward', () => {
        rewarded = true
      })

      // 監聽：廣告關閉
      dismissHandle = AdMob.addListener('onRewardedVideoAdDismissed', () => {
      ...
      // 監聯：顯示失敗
      failShowHandle = AdMob.addListener(
      ...
      // 監聽：載入失敗
      failLoadHandle = AdMob.addListener(
  ...
  async _showInternalAd() {
    ...
    return new Promise(resolve => {
      ...
      modal.className =
        'fixed inset-0 bg-black/50 z-[70] flex items-center justify-center p-4 animation-fade-in'
      modal.innerHTML = `
  ...
  _showConfirmModal(onConfirm) {
    ...
    this._modal
      .querySelector('#reward-confirm-btn')
      .addEventListener('click', () => {
        this._dismissModal()
        onConfirm()
      })
    ...
    this._modal
      .querySelector('#reward-cancel-btn')
      .addEventListener('click', () => {
        this._dismissModal()
        this._cleanupRewardedSlot()
        this._safeResolve(false)

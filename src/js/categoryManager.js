...
export class CategoryManager {
    ...
    async init() {
        if (!this.dataService) {
            console.warn(
                'CategoryManager: DataService not provided, falling back to empty categories'
            )
            return
        }
        ...
        try {
            let saved =
                await this.dataService.getCategorySetting('custom_categories')
            ...
            const order =
                await this.dataService.getCategorySetting('category_order')
            if (order && order.value) this.categoryOrder = order.value

            const hidden =
                await this.dataService.getCategorySetting('hidden_categories')
            ...
            if (this.dataService) {
                ...
                if (!skipLog) {
                    ...
                    this.dataService.logChange(
                        'update',
                        `category_order_${ledgerId}`,
                        'all',
                        this.categoryOrder
                    )
                    this.dataService.logChange(
                        'update',
                        `hidden_categories_${ledgerId}`,
                        'all',
                        this.hiddenCategories
                    )
                }
    ...
    getCustomCategoryById(type, categoryId) {
        return (
            this.customCategories[type]?.find(cat => cat.id === categoryId) ||
            null
        )
    }

    getCategoryById(type, categoryId) {
        const defaultCategory = CATEGORIES[type]?.find(
            cat => cat.id === categoryId
        )
    ...
    isCustomCategory(type, categoryId) {
        return (
            this.customCategories[type]?.some(cat => cat.id === categoryId) ||
            false
        )
    }
                ...
                .addEventListener('click', async () => {
                    ...
                    const proceedFallback = async () => {
                        ...
                        if (this.categoryOrder[type]) {
                            this.categoryOrder[type] = this.categoryOrder[
                                type
                            ].filter(id => id !== categoryId)
                        }
                        if (this.hiddenCategories[type]) {
                            this.hiddenCategories[type] = this.hiddenCategories[
                                type
                            ].filter(id => id !== categoryId)
                        }
    ...
    `
        ...
        document.querySelectorAll('.delete-category-btn').forEach(btn => {
            btn.addEventListener('click', async e => {
                e.stopPropagation()
                const categoryId = btn.dataset.categoryId
                const res = await this.deleteCategoryWithFallback(
                    type,
                    categoryId,
                    onUpdateCallback
                )
        ...
        document.querySelectorAll('.edit-category-btn').forEach(btn => {
            btn.addEventListener('click', e => {
                e.stopPropagation()
                const categoryId = btn.dataset.categoryId
                const categoryToEdit = this.getCustomCategoryById(
                    type,
                    categoryId
                )
                if (categoryToEdit) {

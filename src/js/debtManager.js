    ...
    async updateSummaryCards() {
        ...
        if (this.currentContactFilter) {
            filteredDebts = allDebts.filter(
                d => String(d.contactId) === String(this.currentContactFilter)
            )
        }
        ...
        const selectedContact = this.currentContactFilter
            ? contacts.find(
                  c => String(c.id) === String(this.currentContactFilter)
              )?.name || '聯絡人'
            : null
    ...
    setupEventListeners() {
        ...
        this.container.querySelectorAll('.debt-filter-btn').forEach(btn => {
            btn.addEventListener('click', async e => {
                ...
                // Update UI
                this.container
                    .querySelectorAll('.debt-filter-btn')
                    .forEach(b => {
                        b.classList.remove(
                            'bg-wabi-surface',
                            'text-wabi-primary',
                ...
                if (await customConfirm('確定要刪除此欠款記錄嗎？')) {
                    ...
                    if (recordId) {
                        if (
                            ...
                            // 清除紀錄上的反向引用，避免留下孤立指標
                            await this.dataService.updateRecord(recordId, {
                                debtId: null,
                            })
        ...
        modal
            ...
            .addEventListener('click', async () => {
                const amount = parseFloat(
                    modal.querySelector('#partial-amount').value
                )
        ...
        modal
            ...
            .addEventListener('click', async () => {
                const contactId = parseInt(
                    modal.querySelector('#debt-contact').value
                )
                const amount = parseFloat(
                    modal.querySelector('#debt-amount').value
                )
                const date = modal.querySelector('#debt-date').value
                const description =
                    modal.querySelector('#debt-description').value
                ...
                if (isEdit) {
                    ...
                    const paidAmount = (debtToEdit.payments || []).reduce(
                        (sum, p) => sum + p.amount,
                        0
                    )

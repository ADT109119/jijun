...
export class PluginManager {
    ...
    _denied(group, permNeeded) {
        const label =
            PluginManager.PERMISSION_LABELS[permNeeded]?.label || permNeeded
        return new Proxy(
            {},
            {
                get: (_, prop) => () => {
                    throw new Error(
                        `Permission Denied: 此插件未取得「${label}」權限，無法呼叫 context.${group}.${prop}()`
                    )
                },
            }
        )
    ...
    createPluginContext(pluginId, permissions = [], initializedStorage = null) {
        ...
        // ---- storage ----
        const storage =
            has('storage') && pluginId && initializedStorage
                ? initializedStorage
        ...
        const dataRead = has('data:read')
            ? {
                  ...
                  getCategories: type =>
                      this.app.categoryManager.getAllCategories(type),
                  getCategory: (type, id) =>
                      this.app.categoryManager.getCategoryById(type, id),
              }
            : {}
        ...
        const dataWrite = has('data:write')
            ? {
                  ...
                  updateRecord: (id, updates) =>
                      this.dataService.updateRecord(id, updates),
                  updateDebt: (id, updates) =>
                      this.dataService.updateDebt(id, updates),
              }
            : {}

        // 合併讀寫，未授權的方法以 Proxy 攔截
        const dataApi =
            has('data:read') || has('data:write')
                ? { ...dataRead, ...dataWrite }
        ...
        const uiApi = has('ui')
            ? {
                  ...
                  registerPage: (routeId, title, renderFn) =>
                      this.registerPage(routeId, title, renderFn),
                  registerHomeWidget: (id, renderFn) =>
                      this.registerHomeWidget(id, renderFn, pluginId),
                  navigateTo: hash => {
                  ...
                  showAlert: (title, message) =>
                      this.showAlertModal(title, message),
              }
            : this._denied('ui', 'ui')
    ...
    async loadInstalledPlugins() {
        try {
            ...
            console.warn(
                'Plugins store access failed (might be first run with new version):',
                e
            )
        }
    }
    ...
    async loadPlugin(pluginData) {
        ...
        `
            ...
            const blob = new Blob([sandboxedScript], {
    ...
    static PERMISSION_LABELS = {
        storage: {
            icon: 'fa-database',
            label: '儲存空間',
            desc: '允許在本機存取插件專屬的儲存空間',
        },
        'data:read': {
            icon: 'fa-eye',
            label: '讀取帳務資料',
            desc: '允許讀取記帳紀錄、帳戶、欠款、分類等資料',
        },
        'data:write': {
            icon: 'fa-pen-to-square',
            label: '寫入帳務資料',
            desc: '允許新增或修改記帳紀錄、欠款、聯絡人',
        },
        ui: {
            icon: 'fa-window-maximize',
            label: '使用者介面',
            desc: '允許註冊頁面、顯示通知、首頁小工具',
        },
        network: {
            icon: 'fa-globe',
            label: '網路存取',
            desc: '允許與外部伺服器通訊（如匯率查詢）',
        },
        camera: {
            icon: 'fa-camera',
            label: '相機權限',
            desc: '允許透過相機掃描條碼或讀取影像',
        },
    }
    ...
    showPermissionConsent(meta, permissions = [], isUpdate = false) {
        return new Promise(resolve => {
            ...
            const permListHtml =
                permissions.length > 0
                    ? permissions
                          .map(p => {
                              const info = PluginManager.PERMISSION_LABELS[
                                  p
                              ] || {
                                  icon: 'fa-question',
                                  label: p,
            ...
            modal.className =
                'fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 animation-fade-in'
            modal.innerHTML = `
      ...
      `
            ...
            modal
                .querySelector('#pm-perm-cancel')
                .addEventListener('click', () => close(false))
            modal
                .querySelector('#pm-perm-accept')
    ...
    async installPlugin(file, storePluginInfo = null) {
        return new Promise((resolve, reject) => {
            ...
            reader.onload = async e => {
               ...
               ${scriptContent}
             `
                const blob = new Blob([sandboxedValidation], {
                    type: 'text/javascript',
                })
                const url = URL.createObjectURL(blob)
                let meta = {}
                ...
                const permissions =
                    storePluginInfo?.permissions || meta.permissions || []

                // 檢查是否為更新，並比對權限差異
                let existingPlugin = null
                try {
                    const tx = this.dataService.db.transaction(
                        'plugins',
                        'readonly'
                    )
                ...
                const pluginData = {
                    ...
                    installedAt: existingPlugin
                        ? existingPlugin.installedAt
                        : Date.now(),
                    ...(existingPlugin && existingPlugin.storage
                        ? { storage: existingPlugin.storage }
                        : {}),
                }
    ...
    registerPage(routeId, title, renderFn) {
        if (this.customPages.has(routeId)) {
            console.warn(
                `Plugin page route '${routeId}' already exists. Overwriting.`
            )
        }
    ...
    registerHomeWidget(id, renderFn, pluginId) {
        if (typeof id !== 'string') {
            console.warn(
                'registerHomeWidget now expects (id, renderFn). Ignoring registration.'
            )
            return
        }
    ...
    async moveWidget(id, direction) {
        ...
        if (newIndex < 0 || newIndex >= this.widgetOrder.length) return // Swap
        ;[this.widgetOrder[index], this.widgetOrder[newIndex]] = [
            this.widgetOrder[newIndex],
            this.widgetOrder[index],
        ]
    ...
    showConfirmModal(title, message) {
        return new Promise(resolve => {
          ...
          `
            ...
            modal
                .querySelector('#pm-modal-cancel')
                .addEventListener('click', () => close(false))
            modal
                .querySelector('#pm-modal-confirm')
                .addEventListener('click', () => close(true))
        })
    }

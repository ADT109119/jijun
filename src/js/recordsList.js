...
export class RecordsListManager {
  ...
  async init() {
    ...
    if (defaultPeriod === 'last') {
      ...
      const savedLast = await this.dataService.getSetting(
        'lastRecordsPeriodState'
      )
      const stateData = savedLast?.value || savedLast
      if (
        stateData?.period &&
        stateData?.customStartDate &&
        stateData?.customEndDate
      ) {
  ...
  setupEventListeners() {
    ...
    if (headerTitle)
      headerTitle.addEventListener('click', () => this.showDateRangeModal())
  ...
  applyFiltersAndRender() {
    ...
    this.container.querySelector('#total-income').textContent = formatCurrency(
      summary.income
    )
    this.container.querySelector('#total-expense').textContent = formatCurrency(
      summary.expense
    )
  ...
  showCategoryFilterModal() {
    ...
    this.modalsContainer
      ...
      .addEventListener('click', () => (this.modalsContainer.innerHTML = ''))

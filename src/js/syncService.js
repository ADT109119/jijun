      ...
      const newPayments = await Promise.all(
        data.payments.map(async p => {
          if (p.recordUuid) {
            ...
            return { ...p, recordId: rec ? rec.id : null }
          }
          return p
        })
      )
    ...
    switch (storeName) {
      ...
      case 'debts': {
        ...
        if (
          ...
          const promises = resolvedDebt.payments
            .filter(p => p.recordId)
            .map(p =>
              this.dataService.updateRecord(
                p.recordId,
                { debtId: debtId },
                true
              )
      ...
      case 'debts': {
        ...
        if (resolvedDebt.payments && Array.isArray(resolvedDebt.payments)) {
          const promises = resolvedDebt.payments
            .filter(p => p.recordId)
            .map(p =>
              this.dataService.updateRecord(p.recordId, { debtId: id }, true)
            )
          await Promise.all(promises)
        }
        break

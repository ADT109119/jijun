...
export class StatisticsManager {
  ...
  renderExpenseDonutChart(expenseData) {
    ...
    const colors = categoryData.map(c => {
      ...
      if (match) {
        ...
        // const colorValue = match[2]; // unused
  ...
  async renderAnnualHeatmap(dateRange) {
    ...
    for (let i = 0; i < daysInYear + (7 - (daysInYear % 7)); i++) {
      ...
      // Default color
      cell.className =
        'size-3 rounded-sm bg-gray-100 transition-all hover:ring-2 hover:ring-red-300 cursor-pointer relative group'
      ...
      if (amount > 0) {
        ...
        if (intensity > 0.75)
          cell.className =
            'size-3 rounded-sm bg-red-700 transition-all hover:ring-2 hover:ring-red-300 cursor-pointer'
        else if (intensity > 0.5)
          cell.className =
            'size-3 rounded-sm bg-red-500 transition-all hover:ring-2 hover:ring-red-300 cursor-pointer'
        else if (intensity > 0.25)
          cell.className =
            'size-3 rounded-sm bg-red-300 transition-all hover:ring-2 hover:ring-red-300 cursor-pointer'
        else
          cell.className =
            'size-3 rounded-sm bg-red-100 transition-all hover:ring-2 hover:ring-red-300 cursor-pointer'
      }

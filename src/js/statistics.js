...
export class StatisticsManager {
    ...
    async renderStatisticsPage(container) {
        ...
        const accountFilterOptions = this.accounts
            .map(
                acc =>
    ...
    setupEventListeners() {
        ...
        if (this.advancedModeEnabled) {
            const accountFilter = this.container.querySelector(
                '#stats-account-filter'
            )
            if (accountFilter) {
                accountFilter.addEventListener('change', e => {
                    this.filters.selectedAccountId =
                        e.target.value === 'all'
                            ? null
    ...
    async loadStatisticsData() {
        ...
        const accountBalanceCard = this.container.querySelector(
            '#stats-account-balance-card'
        )
        if (
            this.advancedModeEnabled &&
            this.filters.selectedAccountId !== null
        ) {
            const account = this.accounts.find(
                a => a.id === this.filters.selectedAccountId
            )
            if (account) {
                const allRecordsForAccount = await this.dataService.getRecords({
                    accountId: this.filters.selectedAccountId,
                })
                const currentBalance = allRecordsForAccount.reduce(
                    (balance, record) => {
                ...
                const accountBalanceEl = accountBalanceCard.querySelector(
    ...
    renderTopExpenses(records) {
        ...
        const container = this.container.querySelector(
            '#stats-top-expenses-list'
        )
        if (!container) return

        if (topList.length === 0) {
            container.innerHTML =
                '<p class="text-sm text-center text-wabi-text-secondary">無消費紀錄</p>'
            return
        }
    ...
    updateSummaryCards(stats) {
        ...
        this.container.querySelector('#stats-total-income').textContent =
            formatCurrency(stats.totalIncome)
        this.container.querySelector('#stats-total-expense').textContent =
            formatCurrency(stats.totalExpense)
        this.container.querySelector('#stats-net-balance').textContent =
            formatCurrency(netBalance)
        this.container.querySelector('#stats-net-balance').className =
            `text-2xl font-bold tracking-tight ${netBalance >= 0 ? 'text-wabi-income' : 'text-wabi-expense'}`
    }

    renderTrendChart(dailyData, dateRange) {
        const ctx = this.container
            .querySelector('#stats-trend-chart')
            .getContext('2d')
    ...
    renderExpenseDonutChart(expenseData) {
        ...
        const categoryData = Object.keys(expenseData)
            .map(id => {
                const category = this.categoryManager.getCategoryById(
                    'expense',
                    id
        ...
        const ctx = this.container
            .querySelector('#stats-expense-donut-chart')
            .getContext('2d')
        ...
        // Custom Legend
        const legendContainer = this.container.querySelector(
            '#stats-expense-legend'
        )
    ...
    renderIncomeDonutChart(incomeData) {
        ...
        const categoryData = Object.keys(incomeData)
            .map(id => {
                const category = this.categoryManager.getCategoryById(
                    'income',
                    id
        ...
        const ctx = this.container
            .querySelector('#stats-income-donut-chart')
            .getContext('2d')
    ...
    async renderAnnualHeatmap(dateRange) {
        ...
        for (let i = 0; i < daysInYear + (7 - (daysInYear % 7)); i++) {
            // Fill up the grid columns
            const currentDate = new Date(
                startDate.getTime() + i * dayMilliseconds
            )
            ...
            // Default color
            cell.className =
                'size-3 rounded-sm bg-wabi-border transition-all hover:ring-2 hover:ring-wabi-expense/50 cursor-pointer relative group'
            ...
            if (amount > 0) {
                ...
                if (intensity > 0.75)
                    cell.className =
                        'size-3 rounded-sm bg-wabi-expense opacity-100 transition-all hover:ring-2 hover:ring-wabi-expense/50 cursor-pointer'
                else if (intensity > 0.5)
                    cell.className =
                        'size-3 rounded-sm bg-wabi-expense opacity-75 transition-all hover:ring-2 hover:ring-wabi-expense/50 cursor-pointer'
                else if (intensity > 0.25)
                    cell.className =
                        'size-3 rounded-sm bg-wabi-expense opacity-50 transition-all hover:ring-2 hover:ring-wabi-expense/50 cursor-pointer'
                else
                    cell.className =
                        'size-3 rounded-sm bg-wabi-expense opacity-25 transition-all hover:ring-2 hover:ring-wabi-expense/50 cursor-pointer'
            }

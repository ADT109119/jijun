...
export class PluginStorage {
    ...
    constructor(pluginId, dataService) {
        ...
        if (!/^[a-zA-Z0-9._-]+$/.test(pluginId)) {
            console.warn(
                `PluginStorage: Sub-optimal pluginId format: "${pluginId}". Only alphanumeric, dots, underscores, and hyphens are recommended.`
            )
        }
    ...
    async init() {
        ...
        try {
            ...
            console.error(
                `[PluginStorage] Failed to read plugin data from DB for ${this.pluginId}`,
                e
            )
        }

    ...
    async _renderList(items) {
        const filtered =
            this.statusFilter === 'all'
                ? items
                : items.filter(a => a.status === this.statusFilter)
        ...
        const htmls = await Promise.all(
            filtered.map(async item => {
                ...
                const status =
                    STATUS_LABELS[item.status] || STATUS_LABELS.active
                const progress =
                    item.periods > 0
                        ? Math.round(
                              (item.completedPeriods / item.periods) * 100
                          )
                        : 0
    ...
    _setupListeners() {
        ...
        document.querySelectorAll('.toggle-pause-btn').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                await this.app.dataService.updateAmortization(
                    parseInt(btn.dataset.id),
                    { status: newStatus }
                )
                showToast(
                    newStatus === 'paused' ? '已暫停' : '已恢復',
                    'success'
                )
                await this.render()
            })
        ...
        document.querySelectorAll('.delete-amort-btn').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                if (
                    ...
                    return
                await this.app.dataService.deleteAmortization(
                    parseInt(btn.dataset.id)
                )

...
export class RecurringPage {
    ...
    async setupRecurringPageListeners() {
        ...
        const recurringTxs =
            await this.app.dataService.getRecurringTransactions()
    ...
    async showRecurringTransactionModal(txToEdit = null) {
        ...
        const advancedModeSetting = await this.app.dataService.getSetting(
            'advancedAccountModeEnabled'
        )
        const advancedModeEnabled = !!advancedModeSetting?.value
        const accounts = advancedModeEnabled
            ? await this.app.dataService.getAccounts()
            : []

        // Prepare category and account options
        const expenseCategories = this.app.categoryManager
            .getAllCategories('expense')
            .map(c => `<option value="${c.id}">${escapeHTML(c.name)}</option>`)
            .join('')
        const incomeCategories = this.app.categoryManager
            .getAllCategories('income')
            .map(c => `<option value="${c.id}">${escapeHTML(c.name)}</option>`)
            .join('')
        const accountOptions = advancedModeEnabled
            ? accounts
                  .map(
                      acc =>
        ...
        const skipMonthlyControls = modal.querySelector(
            '#skip-monthly-controls'
        )
        ...
        const updateCategoryOptions = () => {
            if (currentType === 'expense') {
                ...
                typeExpenseBtn.classList.add(
                    'bg-wabi-expense',
                    'text-wabi-surface'
                )
                typeIncomeBtn.classList.remove(
                    'bg-wabi-income',
                    'text-wabi-surface'
                )
            } else {
                categorySelect.innerHTML = incomeCategories
                typeIncomeBtn.classList.add(
                    'bg-wabi-income',
                    'text-wabi-surface'
                )
                typeExpenseBtn.classList.remove(
                    'bg-wabi-expense',
                    'text-wabi-surface'
                )
            }
        ...
        if (txToEdit) {
            ...
            if (advancedModeEnabled)
                modal.querySelector('#recurring-account').value =
                    txToEdit.accountId
            ...
            if (txToEdit.skipRules && Array.isArray(txToEdit.skipRules)) {
                txToEdit.skipRules.forEach(rule => {
                    ...
                    if (type === 'dayOfWeek') {
                        ...
                        modal.querySelector('#skip-month-of-year-input').value =
                            values.map(m => m + 1).join(', ')
                    }
                })
            }
        }
        ...
        modal
            ...
            .addEventListener('click', async () => {
                const data = {
                    ...
                    amount: parseFloat(
                        modal.querySelector('#recurring-amount').value
                    ),
                    type: currentType,
                    category: categorySelect.value,
                    accountId: advancedModeEnabled
                        ? parseInt(
                              modal.querySelector('#recurring-account').value,
                              10
                          )
                    ...
                    startDate: modal.querySelector('#recurring-start-date')
                        .value,
                    nextDueDate: modal.querySelector('#recurring-start-date')
                        .value, // First due date is the start date
                    skipRules: [],
                }
                ...
                const weeklyValues = [
                    ...modal.querySelectorAll(
                        'input[name="skipDayOfWeek"]:checked'
                    ),
                ...
                if (isEdit) {
                    await this.app.dataService.updateRecurringTransaction(
                        txToEdit.id,
                        {
                            ...txToEdit,
                            ...data,

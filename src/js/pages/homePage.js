...
export class HomePage {
    ...
    setupEventListeners() {
        const monthSelectorBtn = document.getElementById(
            'home-month-selector-btn'
        )
        if (monthSelectorBtn) {
            monthSelectorBtn.addEventListener('click', () => {
                const currentMonthDisplay =
                    document.getElementById('home-month-display').textContent
                const [year, month] = currentMonthDisplay
                    .split(' / ')
                    .map(Number)
                this.showMonthYearPickerModal(
                    year,
                    month - 1,
                    newMonthString => {
    ...
    showWidgetOrderModal() {
        ...
        const renderList = () => {
            ...
            return activeWidgets
                .map((id, index) => {
                    const name =
                        this.app.pluginManager.getPluginName(id) || '未知小工具'
            ...
            modal.querySelectorAll('.toggle-hide-btn').forEach(btn => {
                btn.addEventListener('click', async e => {
                    ...
                    if (hiddenWidgets.includes(id)) {
                        this.app.pluginManager.hiddenWidgets =
                            hiddenWidgets.filter(wId => wId !== id)
                    } else {
        ...
        modal
            .querySelector('#close-widget-modal')

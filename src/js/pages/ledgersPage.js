...
export class LedgersPage {
  ...
  _showEditModal(ledger) {
                ...
                <div class="mb-4">
                    ...
                    <div id="color-picker" class="flex flex-wrap gap-2">
                        ...
                        `
                          ...
                          .join('')}
                        <label for="ledger-custom-color-picker-input" id="ledger-custom-color-picker-label" class="size-8 rounded-full border-2 border-dashed border-wabi-border flex items-center justify-center cursor-pointer hover:border-wabi-primary transition-all">
                            <i class="fas fa-palette text-wabi-text-secondary text-xs"></i>
                            <input type="color" id="ledger-custom-color-picker-input" class="absolute w-0 h-0 opacity-0" value="#888888">
                        </label>
                    </div>
                    ...
                    <div class="mb-3">
                        <div class="flex items-center space-x-2 mb-2">
                            <input type="text" id="ledger-custom-icon-input"
                                ...
                                value="${selectedIcon}"
                                class="flex-1 px-3 py-2 text-sm border border-wabi-border rounded-lg focus:ring-2 focus:ring-wabi-primary focus:border-transparent bg-wabi-surface text-wabi-text-primary outline-none">
                        </div>
                        <input type="text" id="ledger-icon-search-input"
                            placeholder="搜尋內建圖示... (例: heart)"
                            class="w-full px-3 py-2 text-sm border border-wabi-border rounded-lg focus:ring-2 focus:ring-wabi-primary focus:border-transparent bg-wabi-surface text-wabi-text-primary outline-none mb-2">
                    </div>
                    <div id="icon-picker" class="grid grid-cols-8 gap-2 max-h-48 overflow-y-auto border border-wabi-border rounded-lg p-2 bg-wabi-surface">
    ...
    const customColorPickerInput = modal.querySelector(
      '#ledger-custom-color-picker-input'
    )
    const customColorPickerLabel = modal.querySelector(
      '#ledger-custom-color-picker-label'
    )
    ...
    const iconSearchInput = modal.querySelector('#ledger-icon-search-input')
    ...
    const renderIcons = iconsToRender => {
      iconPicker.innerHTML = iconsToRender
        .map(
          ic => `
    ...
    iconSearchInput.addEventListener('input', e => {
      ...
      if (filteredIcons.length === 0) {
        iconPicker.innerHTML =
          '<div class="col-span-8 text-center text-sm text-gray-500 py-4">找不到相關圖示</div>'
      } else {

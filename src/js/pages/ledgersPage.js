...
export class LedgersPage {
    ...
    _setupListeners() {
        ...
        document
            ...
            ?.addEventListener('click', () => {
                if (
                    !this.app.syncService ||
                    !this.app.syncService.isSignedIn()
                ) {
    ...
    _showEditModal(ledger) {
        ...
        const selectColor = color => {
            ...
            // 嘗試高亮匹配的預設色
            const matched = modal.querySelector(
        ...
        iconSearchInput.addEventListener('input', () => {
            ...
            searchTimeout = setTimeout(() => {
                ...
                if (!keyword) {
                    // 重置為預設圖示
                    iconPicker.innerHTML = defaultIcons
                        .map(
                            ic => `
                        <button class="icon-option size-10 rounded-lg flex items-center justify-center text-lg transition-all
                ...
                const results = FONT_AWESOME_ICONS.filter(i =>
                    i.includes(keyword)
                ).slice(0, 100)
                if (results.length === 0) {
                    iconPicker.innerHTML =
                        '<p class="text-xs text-wabi-text-secondary col-span-6 text-center py-4">找不到符合的圖示</p>'
                } else {
                    iconPicker.innerHTML = results
                        .map(
                            ic => `
                        <button class="icon-option size-10 rounded-lg flex items-center justify-center text-lg transition-all
                            ${ic === iconInput.value ? 'bg-wabi-primary text-wabi-surface shadow-sm' : 'bg-wabi-bg text-wabi-text-secondary hover:bg-wabi-bg'}"
        ...
        // ==================== 自訂圖示 class ====================
        modal
            .querySelector('#apply-custom-icon-btn')
            .addEventListener('click', () => {
            ...
            if (isOwner) {
                modal
                    ...
                    ?.addEventListener('click', async () => {
                        ...
                        try {
                            await this.app.ledgerManager.unshareLedger(
                                ledger.id
                            )
                            showToast(
                                '已取消共用，帳本已還原為個人帳本',
            ...
            submitBtn?.addEventListener('click', async () => {
                ...
                try {
                    ...
                    this._showShareModal(
                        await this.app.dataService.getLedger(ledger.id)
                    )
                } catch (e) {
        ...
        submitBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (code !== authorizedCode) {
                    ...
                    code =
                        await this.app.syncService.openSharedLedgerPicker(code)
                    authorizedCode = code
                ...
                const newLedgerId =
                    await this.app.ledgerManager.joinSharedLedger(code)

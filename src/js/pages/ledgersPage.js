...
export class LedgersPage {
  ...
  _setupListeners() {
    // 新增帳本
    document
      .getElementById('add-ledger-btn')
      ?.addEventListener('click', () => this._showEditModal(null))

    // 加入共用帳本
    document
      .getElementById('join-ledger-btn')
    ...
    document.querySelectorAll('.edit-ledger-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const id = parseInt(btn.dataset.id)
        const ledger = this.app.ledgerManager
          .getAllLedgers()
          .find(l => l.id === id)
    ...
    document.querySelectorAll('.delete-ledger-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (
          !confirm(
            `確定要刪除「${ledger.name}」帳本嗎？\n\n⚠️ 此操作不可復原，帳本內的所有記帳資料、帳戶、欠款等都會一併刪除。`
          )
        )
          return
        try {
  ...
  _showEditModal(ledger) {
    ...
    modal.className =
      'fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 backdrop-blur-[2px]'
    modal.innerHTML = `
                ...
                <div class="mb-4">
                    ...
                    <div id="color-picker" class="flex flex-wrap gap-2">
                        ${colors
                          .map(
                            c => `
                            <button type="button" class="color-option size-8 rounded-full border-2 transition-all ${c === selectedColor ? 'border-wabi-primary scale-110 ring-2 ring-wabi-primary/30' : 'border-transparent hover:scale-110'}" data-color="${c}" style="background-color: ${c}"></button>
                        `
                          )
                          .join('')}
                        <label for="custom-color-picker-input" id="custom-color-picker-label" class="size-8 rounded-full border-2 border-dashed border-wabi-border flex items-center justify-center cursor-pointer hover:border-wabi-primary transition-all relative">
                    ...
                    <div class="mb-3">
                        <div class="flex items-center space-x-2 mb-2">
                            <input type="text" id="custom-icon-input"
                                ...
                                value="${selectedIcon}"
                                class="flex-1 p-2 text-sm bg-transparent border border-wabi-border rounded-lg focus:ring-2 focus:ring-wabi-primary focus:border-transparent text-wabi-text-primary">
                        </div>
                        <input type="text" id="icon-search-input"
                            placeholder="搜尋內建圖示... (例: heart)"
                            class="w-full p-2 text-sm bg-transparent border border-wabi-border rounded-lg focus:ring-2 focus:ring-wabi-primary focus:border-transparent text-wabi-text-primary mb-2">
                    </div>
                    <div id="icon-picker" class="grid grid-cols-8 gap-2 max-h-32 overflow-y-auto border border-wabi-border rounded-lg p-2">
    ...
    const customColorPickerInput = modal.querySelector(
      '#custom-color-picker-input'
    )
    const customColorPickerLabel = modal.querySelector(
    ...
    // 圖示選擇
    const customIconInput = modal.querySelector('#custom-icon-input')
    const iconSearchInput = modal.querySelector('#icon-search-input')
    const iconSelector = modal.querySelector('#icon-picker')

    const updateIconFromInput = () => {
      ...
      if (iconClass) {
        ...
        const matchedBtn = modal.querySelector(
          `.icon-option[data-icon="${iconClass}"]`
        )
        if (matchedBtn) {
          matchedBtn.classList.remove('bg-wabi-bg', 'text-wabi-text-secondary')
          matchedBtn.classList.add(
            'bg-wabi-primary',
            'text-wabi-surface',
            'shadow-sm'
          )
    ...
    customIconInput.addEventListener('input', updateIconFromInput)
    customIconInput.addEventListener('keyup', updateIconFromInput)
    ...
    const renderIcons = iconList => {
      iconSelector.innerHTML = iconList
        .map(
          ic => `
                <button type="button" class="icon-option size-10 rounded-lg flex items-center justify-center text-lg transition-all
    ...
    iconSearchInput.addEventListener('input', e => {
      ...
      const filteredIcons = FONT_AWESOME_ICONS.filter(icon =>
        icon.toLowerCase().includes(query)
      ).slice(0, 50)
      if (filteredIcons.length === 0) {
        iconSelector.innerHTML =
          '<div class="col-span-8 text-center text-sm text-gray-500 py-4">找不到圖示</div>'
      } else {
    ...
    // 取消
    modal
      .querySelector('#ledger-cancel-btn')
  ...
  async _showShareModal(ledger) {
    ...
    if (ledger.sharedFileId) {
      ...
      this.app.ledgerManager
        .getSharedUsers(ledger.id)
        .then(users => {
          usersListEl.innerHTML = ''
          users.forEach(u => {
            const el = document.createElement('div')
            el.className =
              'flex justify-between items-center bg-wabi-surface p-2 rounded-lg border border-wabi-border shadow-sm'
            el.innerHTML = `
                        ...
                        ${
                          u.role === 'owner'
                            ? '<span class="text-[10px] bg-blue-100 text-blue-600 px-2 py-0.5 rounded shrink-0">擁有者</span>'
                            : isOwner
            ...
            if (isOwner && u.role !== 'owner') {
              el.querySelector('.remove-user-btn')?.addEventListener(
                'click',
                async () => {
                  if (
                    !confirm(
                      `確定要移除「${u.emailAddress || u.displayName}」的共用權限嗎？`
                    )
                  )
                    return
                  try {
                    el.style.opacity = '0.5'
                    await this.app.ledgerManager.removeSharedUser(
                      ledger.id,
                      u.id
                    )
                    el.remove()
      ...
      if (isOwner) {
        modal
          .querySelector('#unshare-btn')
          ?.addEventListener('click', async () => {
            if (
              !confirm(
                `⚠️ 確定要取消共用「${ledger.name}」嗎？\n\n所有參與者將失去存取權限，雲端共享檔案將被永久刪除。帳本資料仍會保留在您本地。`
              )
            )
              return
            ...
            btn.innerHTML =
              '<i class="fa-solid fa-spinner fa-spin mr-1"></i>處理中...'
            btn.disabled = true
    ...
    if (isOwner) {
      ...
      submitBtn?.addEventListener('click', async () => {
        ...
        try {
          const fileId = await this.app.ledgerManager.shareLedger(
            ledger.id,
            email
          )
  ...
  _showJoinModal() {
    ...
    pickerBtn.addEventListener('click', async () => {
      ...
      pickerBtn.innerHTML =
        '<i class="fa-solid fa-spinner fa-spin mr-2"></i>載入視窗...'
      pickerBtn.disabled = true

      try {
        const selectedFileId =
          await this.app.syncService.openSharedLedgerPicker()
    ...
    submitBtn.addEventListener('click', async () => {
      ...
      const originalHTML = submitBtn.innerHTML
      submitBtn.innerHTML =
        '<i class="fa-solid fa-spinner fa-spin mr-2"></i>正在處理...'
      submitBtn.disabled = true
      ...
      try {
        ...
        submitBtn.innerHTML =
          '<i class="fa-solid fa-spinner fa-spin mr-2"></i>正在同步...'

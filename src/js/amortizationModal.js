 ...
 */
export function showAmortizationModal(
    app,
    item = null,
    prefill = {},
    onSaved = null
    ...
    const populateCategories = () => {
        ...
        categorySelect.innerHTML =
            ...
            categories
                .map(
                    c =>
                        `<option value="${c.id}" ${defaults.category === c.id ? 'selected' : ''}>${escapeHTML(c.name)}</option>`
                )
                .join('')
    }
    populateCategories()
    ...
    const totalWithInterestDisplay = modal.querySelector(
        '#amort-total-with-interest'
    )
    ...
    const updateCalculation = async () => {
        ...
        if (total <= 0 || periods <= 0) {
            perPeriodDisplay.textContent = '--'
            totalWithInterestDisplay.innerHTML = ''
            return
        }
        const principal = Math.max(0, total - downPayment)
        const decimalStrategy = modal.querySelector(
            '#amort-decimal-strategy'
        ).value
        const frequency = modal.querySelector('#amort-frequency').value

        const { amountPerPeriod, exactTotalToPay } =
            calculateAmortizationDetails(
                principal,
                periods,
                annualRate,
        ...
        if (annualRate > 0) {
            ...
            message =
                downPayment > 0
                    ? `首付 $${fmt(downPayment)} + ${periods} 期`
                    : ''
        }

        if (isEdit && item && item.id) {
            const historyRecords = await app.dataService.getRecords({
    ...
    ;[totalInput, periodsInput, downPaymentInput, interestInput].forEach(el =>
        el.addEventListener('input', updateCalculation)
    )
    modal
        .querySelector('#amort-frequency')
        .addEventListener('change', updateCalculation)
    modal
        .querySelector('#amort-decimal-strategy')
        .addEventListener('change', updateCalculation)
    updateCalculation()

    // ==================== 儲存 ====================
    modal
        .querySelector('#amort-save-btn')
        .addEventListener('click', async () => {
            ...
            const data = {
                ...
                periods,
                completedPeriods: isEdit
                    ? parseInt(
                          modal.querySelector('#amort-completed')?.value
                      ) || 0
                    : 0,
                amountPerPeriod,
                frequency,
            ...
            try {
                if (isEdit) {
                    if (data.completedPeriods >= data.periods)
    ...
    modal
        .querySelector('#amort-cancel-btn')

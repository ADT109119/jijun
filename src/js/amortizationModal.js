...
export function showAmortizationModal(
  ...
  const updateCalculation = async () => {
    ...
    const { amountPerPeriod, exactTotalToPay } = calculateAmortizationDetails(
      principal,
      periods,
      annualRate,
      frequency,
    ...
    if (annualRate > 0) {
      ...
      message =
        downPayment > 0 ? `首付 $${fmt(downPayment)} + ${periods} 期` : ''
    }
  ...
  modal.querySelector('#amort-save-btn').addEventListener('click', async () => {
    ...
    const data = {
      ...
      completedPeriods: isEdit
        ? parseInt(modal.querySelector('#amort-completed')?.value) || 0
        : 0,
      amountPerPeriod,
      frequency,
      decimalStrategy,

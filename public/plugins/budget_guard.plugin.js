export default {
    meta: {
        ...
        permissions: ['storage', 'data:read', 'data:write', 'ui'],
    },
    init(context) {
        ...
        context.ui.registerPage('budget-guard', '預算阻擋者設定', container =>
        ...
        context.events.on('onRecordSaveBefore', async record => {
            if (record.type === 'expense') {
                ...
                if (record.amount > limit) {
                    if (this.mode === 'block') {
                        // Show Blocking Modal
                        await context.ui.showAlert(
                            '⚠️ 預算阻擋',
                            `該筆消費 $${record.amount} 超過單筆上限 $${limit}！\n\n請調整金額。`
                        )
        ...
        console.log(
            'Budget Guard initialized with limit:',
            this.limit,
            'mode:',
            this.mode
        )

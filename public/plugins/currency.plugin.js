export default {
    ...
    injectButton() {
        ...
        btn.className = 'text-sm text-wabi-primary bg-wabi-primary/10 px-3 py-1.5 rounded-lg hover:bg-wabi-primary/20 transition-colors flex items-center gap-2';
    ...
    async openConverter() {
        ...
        modal.className = 'fixed inset-0 bg-black bg-opacity-50 z-[70] flex items-center justify-center p-4 animation-fade-in';
        modal.innerHTML = `
        ...
        const renderButtons = () => {
             ...
             this.currencies.forEach(c => {
                 ...
                 const btn = document.createElement('button');
                 btn.className = 'border border-gray-200 rounded-xl p-3 hover:bg-blue-50 hover:border-blue-300 transition-all text-left group active:scale-95';
                 btn.innerHTML = `
    ...
    applyAmount(twd, code, foreign) {
        // Read Check Type
        let type = 'expense';
        if (document.getElementById('add-type-income')?.classList.contains('bg-wabi-income')) {
            type = 'income';
        }
        ...
        const note = currentNote ? `${currentNote} (${foreign} ${code})` : `${foreign} ${code}`;

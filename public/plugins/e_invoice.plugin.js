export default {
            ...
            <div class="bg-white rounded-xl w-full max-w-md overflow-hidden flex flex-col shadow-2xl relative">
                ...
                <div class="p-5 flex flex-col items-center w-full">
                    <div class="relative w-full max-w-[320px] aspect-[4/5] bg-black rounded-lg overflow-hidden shadow-inner">
                        <video id="reader-video" class="absolute inset-0 w-full h-full object-cover" playsinline></video>
                        <canvas id="reader-canvas" class="absolute inset-0 w-full h-full object-cover pointer-events-none z-10"></canvas>

                        <div id="reader-loading" class="absolute inset-0 flex flex-col items-center justify-center text-wabi-surface text-sm bg-black z-20 transition-opacity duration-300">
                            <i class="fa-solid fa-spinner fa-spin text-2xl mb-2"></i>
                            <p class="mt-2 tracking-wider">正在啟動高畫質相機...</p>
                        </div>
                    </div>
                    
                    <div id="scan-hint" class="mt-5 flex items-center justify-center gap-2 text-xs text-gray-500 bg-gray-50 px-3 py-2 rounded-lg border border-gray-100 w-full max-w-[280px]">
                        <i class="fa-solid fa-circle-info text-gray-400"></i>
                        <span>請將左側發票 QR Code 置於畫面中</span>
                    </div>
                </div>
            </div>
        `
        ...
        try {
            ...
            if ('BarcodeDetector' in window) {
                try {
                    ...
                    console.warn(
                        'BarcodeDetector init failed, fallback to jsQR',
                        e
                    )
                }
            }
            ...
            const scanLoop = async timestamp => {
                ...
                if (
                    ...
                    ctx.fillRect(
                        ...
                        sy + cropSize,
                        canvas.width,
                        canvas.height - (sy + cropSize)
                    ) // 下遮罩
                    ctx.fillRect(0, sy, sx, cropSize) // 左遮罩
                    ctx.fillRect(
                        sx + cropSize,
                        sy,
                        canvas.width - (sx + cropSize),
                        cropSize
                    ) // 右遮罩
                    ...
                    // 繪製雷射掃描動畫線
                    const laserY =
                        sy + (Math.sin(timestamp / 250) * 0.5 + 0.5) * cropSize
                    ...
                    try {
                        if (barcodeDetector) {
                            const barcodes =
                                await barcodeDetector.detect(cropCanvas)
                    ...
                    for (const result of qrResults) {
                        ...
                        const isLeftInvoice =
                            qrUpper.length >= 37 &&
                            /^[A-Z]{2}\d{15}/.test(qrUpper.substring(0, 17))
                    ...
                    if (!foundValidInvoice && qrResults.length === 0) {
                        if (
                            hintEl &&
                            hintEl.className.includes('bg-yellow-50')
                        ) {
    ...
    async handleScanResult(qrData, closeModal) {
        ...
        try {
            ...
            const sellerBan =
                qrData.length >= 53 ? qrData.substring(45, 53) : ''
            let storeName = ''
    ...
    fillAddForm(invoiceData) {
        ...
        const storeText = invoiceData.storeName
    ...
    async initBackgroundCheck(force = false) {
        ...
        const lastSync =
            (await this.context.storage.getItem('last_sync_winning_numbers')) ||
            0
        const now = Date.now()
    ...
    async fetchWinningNumbers() {
        try {
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                const firstMatch = descText.match(
                    /頭獎：\s*(\d{8}(?:[、，,]\d{8})*)/
                )
    ...
    registerWidget() {
        this.context.ui.registerHomeWidget(
            'e_invoice_widget',
            async container => {
                container.innerHTML = `
        ...
        this.context.ui.registerPage(
            ...
            `
                ...
                const loadData = async () => {
                    try {
                        const storedInv =
                            await this.context.storage.getItem('invoices')
                        if (storedInv) invoices = JSON.parse(storedInv)
                        const storedWin =
                            await this.context.storage.getItem(
                                'winning_numbers'
                            )
                ...
                const renderUI = () => {
                    ...
                    container.querySelector('#period-display').innerText =
                        currentPeriod
                    ...
                    const winContent = container.querySelector(
                    ...
                    container
                        ...
                        .forEach(btn => {
                            btn.addEventListener('click', async e => {
                                const scannedAt = parseInt(
                                    e.currentTarget.dataset.scannedAt,
                                    10
                                )
                                const confirm =
                                    await this.context.ui.showConfirm(
                                        '移除發票',
                                        '確定要移除這張發票嗎？移除後將無法自動對獎，但不會影響已記帳的紀錄。'
                                    )
                                if (confirm) {
                                    invoices = invoices.filter(
                                        inv => inv.scannedAt !== scannedAt
                                    )
                                    await this.context.storage.setItem(
                                        'invoices',
                                        JSON.stringify(invoices)
                ...
                container
                    ...
                    .addEventListener('click', async () => {
                        const btn =
                            container.querySelector('#einvoice-sync-btn')

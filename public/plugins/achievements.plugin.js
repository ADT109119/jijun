export default {
    meta: {
        ...
        icon: 'fa-trophy',
        permissions: ['storage', 'data:read', 'ui'],
    },
    init(context) {
        ...
        this.achievements = [
            ...
            },
            {
                id: 'night_owl',
                name: '夜貓子',
                desc: '在凌晨 00:00 - 04:00 記帳',
                icon: 'fa-moon',
            },
            {
                id: 'early_bird',
                name: '早起的鳥兒',
                desc: '在清晨 05:00 - 08:00 記帳',
                icon: 'fa-sun',
            },
            {
                id: 'big_spender_plus',
        ...
        // Register Page
        context.ui.registerPage('achievements', '成就館', container =>
            this.renderPage(container)
        )

        // Hook for Logic
        context.events.on('onRecordSaveAfter', record =>
            this.checkAchievements(record)
        )

        // Hook for Notification
        context.events.on('onPageRenderAfter', () =>
    ...
    checkAchievements(record) {
        ...
        if (record.type === 'expense') {
            this.stats.totalExpense =
                (this.stats.totalExpense || 0) + record.amount
        }
        ...
        if (record.description && record.description.length >= 20)
            this.unlock('note_taker', newUnlocks)

        // Amount Checks (Expense only)
        if (record.type === 'expense') {
            if (record.amount < 10 && record.amount > 0)
                this.unlock('penny_pincher', newUnlocks)
            if (record.amount > 5000) this.unlock('big_spender', newUnlocks)
            if ((this.stats.totalExpense || 0) > 1000000)
                this.unlock('millionaire', newUnlocks)
        }
        ...
        if (record.type === 'expense' && record.amount > 10000)
            this.unlock('big_spender_plus', newUnlocks)
        ...
        if (newUnlocks.length > 0) {
            const currentPending =
                this.context.storage.getJSON('pending_notify') || []
            const uniquePending = [
                ...new Set([...currentPending, ...newUnlocks]),
            ]

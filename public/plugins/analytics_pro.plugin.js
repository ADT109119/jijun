export default {
  ...
  init(context) {
    this.ctx = context
    this.ctx.ui.registerPage('analytics-pro', '進階分析', container =>
      this.render(container)
    )

    // Add home widget entry
    this.ctx.ui.registerHomeWidget(
      'com.walkingfish.analytics_pro',
      container => {
        container.innerHTML = `
            ...
            `
        container
          .querySelector('#open-analytics-widget')
  ...
  async loadData() {
    ...
    document.getElementById('year-total-expense').textContent =
  ...
  renderHeatmap(records) {
    ...
    for (let i = 0; i < 371; i++) {
      // Slightly more than 366 to fill grid
      ...
      cell.className =
        'size-3 rounded-sm bg-gray-100 transition-all hover:ring-2 hover:ring-indigo-300 cursor-pointer'
      cell.title = `${dateStr}: $${amount}`

      if (amount > 0) {
        const intensity = amount / maxAmount
        if (intensity > 0.75)
          cell.className = 'size-3 rounded-sm bg-emerald-700'
        else if (intensity > 0.5)
          cell.className = 'size-3 rounded-sm bg-emerald-500'
        else if (intensity > 0.25)
          cell.className = 'size-3 rounded-sm bg-emerald-300'
  ...
  tailwindToHex(className) {
    ...
    const name = match[1]
    // const shade = match[2]; // Currently unused, but could be used for shade mapping // eslint-disable-line no-unused-vars
  ...
  async renderAssetTrendChart(records) {
    ...
    // const cutoffDate = new Date(months[0]); // Oldest month end // unused
    ...
    // const balanceHistory = {}; // 'YYYY-MM': balance // unused

    // Current month (partial)
    // balanceHistory[now.toISOString().slice(0, 7)] = runningBalance;

    // Function to get Month Key
    // const getMonthKey = (date) => `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`; // unused

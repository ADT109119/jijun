export default {
    ...
    async loadData() {
        ...
        const totalExpense = thisYearRecords.reduce(
            (sum, r) => sum + r.amount,
            0
        )
    ...
    async renderCategoryChart(records) {
        ...
        Object.entries(categories)
            ...
            .forEach(([catId, amount], index) => {
                ...
                labels.push(
                    catDef ? catDef.name : catId === 'others' ? '其他' : catId
                )
                data.push(amount)
    ...
    async renderCategoryComparisonChart(records) {
        ...
        records.forEach(r => {
            ...
            if (months[key]) {
                months[key][r.category] =
                    (months[key][r.category] || 0) + r.amount
            }
        })

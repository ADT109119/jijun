...
> **⚠️ 顏色值格式限制（v2.1.6.4 起）**：`colors` 的鍵名僅允許英數字、連字號與底線（會自動過濾其他字元）；非十六進位的值（如 `rgba(...)`、`var(...)`、`linear-gradient(...)`、`transparent`）可直接使用，但其中包含的 `;`、`}`、`@`、`<`、`>` 字元會被自動移除以防止 CSS 注入。建議顏色統一使用十六進位格式以獲得最佳相容性。

...
- **深淺色模式自動切換與外觀管理**: 支援跟隨系統（`prefers-color-scheme`）自動即時切換深淺色外觀，並提供「跟隨系統」、「淺色」、「深色」三態等寬切換器，自動同步手機頂部狀態列色彩（`meta theme-color`）。
...
- **AI 離線語音記帳助手**: 整合端側 58M LLM (WASM/wllama@3.5.1) 進行免連網語意 Tool Calling 解析，支援真實 GGUF 模型下載與 OPFS 離線快取，具備前端語意守門（`applySemanticGuardrail`）校準機制防止「花了100元」消費誤判為收入，輸入口述中文自動擷取金額、收支類型、動態分類、帳戶與日期，並搭配高質感霧面玻璃、圓角與 Gemini 彩虹霓虹動態光罩，並提供離線規則引擎作為模型未就緒時的優雅降級機制。
...
- **行事曆金流檢視與 Android 桌面 Widget**: 收支分析頁提供直觀的月曆收支分佈視圖，顯示每日 Top 3 支出分類標籤與金額，並可點擊彈出當日交易明細 Modal；Android 平台同步支援桌面行事曆 Widget，免開 App 即刻概覽當月每日收支事實。
- **真實信用卡分期與智能攤提**: 支援真實信用卡分期（刷卡當下全額入帳佔額度、每月自動產生「扣款帳戶支出 + 信用卡收入」轉帳對還款與額度釋放），整合精準的攤提/分期引擎，自動處理四捨五入尾款與末期差額，支援智能溢繳防護、佔用額度追蹤與編輯防護。
- **首屏 0 第三方 CDN 與極致離線化**: 全站資源（Tailwind CSS、Font Awesome、Google Fonts woff2、QRCode.js、html5-qrcode）改由本地打包，Google SDK 採登入/同步時按需載入，搭配 Service Worker 預快取清單 (`precachemanifest.json`) 與即時離線狀態提示列，實現 100% 離線可用。
- **明細搜尋**: 明細頁面新增搜尋欄位，可依備註內容或交易金額即時篩選記錄。
- **帳戶餘額調整與可用額度**: 帳戶管理頁面新增「平衡」按鈕，輸入實際餘額後自動計算差額，並可選擇自動建立「平帳」調整紀錄以維持帳目正確；信用卡帳戶即時計算並顯示欠款、總額度與可用額度。
- **每月預算設定**: 可設定每月支出預算，水位圖動態顯示預算使用情況，並有超支警告。支援勾選特定支出分類進行「預算排除」，該分類實際消費不計入全局預算統計。
- **自定義分類**: 完整的分類管理功能，可新增自定義分類、選擇圖示和顏色，並與預設分類無縫整合。
- **多帳本分類獨立化**: 自訂分類設定（包含分類、排序與隱藏）由原本全域共享重構為隨帳本獨立儲存。切換帳本時會自動重新載入該帳本專屬的分類設定。
- **信用卡智慧管理與自動扣繳**: 支援信用卡類型帳戶，可自訂信用額度、出帳日與繳款日。系統會在出帳日當天或之後自動計算並產生各期帳單，並支援綁定一般帳戶啟用「自動扣繳」功能。當還款（或轉帳繳卡費）入帳時，透過先進先出 (FIFO) 沖銷演算法自動更新帳單狀態為已繳 (`paid`)。
...
- **PWA 進階原生整合能力**:
    - **Launch Handler (單一實體限制)**: 限制單一執行視窗（`focus-existing`），防止重開分頁時造成 IndexedDB 寫入死鎖。
    - **Share Target (簡訊記帳)**：支援接收外部分享文字，自動擷取簡訊中之金額並預填至記帳頁面的金額與備註中。
    - **Edge / Windows Widget (桌面小工具)**：整合 Windows 桌面小工具面板顯示今日收支事實表，支援 Service Worker 在背景使用原生 IndexedDB 動態統計，並提供一鍵刷新。
- **版本管理**: 設定頁面顯示當前版本號和最後更新時間，支援手動檢查更新功能。
- **完整更新日誌**: 內建版本更新日誌系統，可查看所有歷史版本的詳細更新記錄。
- **可摺疊介面**: 記帳頁面的小鍵盤支援最小化功能，點擊備註列旁的鍵盤圖示按鈕即可切換顯示，緊湊設計節省螢幕空間。
- **智能導覽**: 底部導覽五欄式設計，涵蓋首頁、明細、記帳、統計、設定，核心功能一鍵直達。
- **詳細記錄管理**: 完整的記錄編輯功能，支援修改類型、分類、金額、說明和日期。
- **視覺化增強**: 支出分類甜甜圈圖中央顯示總額，提供直觀的支出概覽。
- **首頁小工具排序**: 支援自訂首頁小工具的排列順序，並可透過眼睛圖示切換各小工具的顯示狀態，讓常用資訊觸手可及。
- **自訂分類功能**: 支援自訂圖標輸入，可使用任何 Font Awesome 圖標類別創建個性化分類。
- **雲端備份與 Per-Device 獨立日誌共用帳本同步**: 整合 Google Drive，個人資料使用專屬備份空間，共用帳本採用先進的 Per-Device 獨立變更日誌（Append-only）與 Manifest 清單架構，透過 ETag CAS 樂觀鎖防競態並支援 Google Drive 權限即時閉環撤銷，徹底杜絕多人並行記帳時的資料覆蓋衝突與時鐘偏差丟失。
- **Google Drive 漸進式授權**: 將雲端同步權限拆分為基礎備份 (`drive.appdata`) 與共用帳本 (`drive.file`)，初次僅要求最低限度基礎權限，保護隱私與提升安全性。
- **擴充功能修改 API**: 於沙箱環境中，對獲得 `data:write` 權限的插件開放 `updateRecord` 與 `updateDebt` API，支援更進階的資料庫實體雙向關聯功能。
- **單元測試基礎建設**: 引入 Vitest + jsdom，覆蓋核心業務邏輯模組，確保重構與修改時的程式碼穩定性。
- **優化輸入體驗**: 記帳面板緊湊設計，日期輸入帶標籤，類別選擇使用統一的綠色主題效果。

### 🧭 導覽系統

- **全方位新手與功能導覽 (Onboarding & Feature Tour)**:
    - **初次開啟歡迎 Modal**: 採用毛玻璃高質感遮罩與 Slider Track 翻頁容器，支援觸控左右滑動手勢、鍵盤方向鍵導航與無障礙 ARIA 標註。
    - **互動式實操記帳引導演練**: 自動引導「新增消費 ➔ 儲存 ➔ 明細查看 ➔ 編輯金額 ➔ 刪除測試紀錄」真實流程，具備中斷保護與資料自動清理，零髒資料殘留。
    - **全功能模組情境引導**: 涵蓋欠款借貸、多帳戶資產、大額分期攤提、群組分帳、AI 語音記帳、預算進度、收支統計、週期交易與多帳本管理等逐步教學。
    - **功能開關即時介紹與重置入口**: 設定頁啟用進階功能時即時彈出功能簡介氣泡，並於「關於」提供「導覽教學」按鈕供隨時重新重溫。
- **底部導覽列**: 優化的五欄式導覽，包含「首頁 (🏠)」、「明細 (📋)」、「記帳 (➕)」、「統計 (📊)」、「設定 (⚙️)」，核心功能一鍵直達。
- **設定頁面**: 整合了資料管理、應用程式管理與版本資訊，提供一站式的功能操作。

### 📱 用戶介面改進

- **計算機模式與可摺疊小鍵盤**: 記帳頁面的數字鍵盤全新升級
    - 支援最小化功能：點擊備註列旁的鍵盤圖示按鈕可切換鍵盤的顯示狀態，隱藏小鍵盤以節省螢幕空間。
    - 內建計算機模式：支援在輸入金額時直接進行加、減、乘、除運算。
    - 設定頁面提供開關以切換是否預設啟用計算機功能。
    - 編輯時自動預設選擇原始分類，提升使用體驗。
    - 鍵盤高度與間距優化，提供平滑的動畫效果和懸停視覺回饋。
- **緊湊輸入面板**: 記帳輸入區域空間優化
    - 日期輸入採用同行布局（標籤與輸入框並排）
...
- **類別選擇優化**: 統一的視覺回饋系統
    ...
    - 無放大效果，保持界面穩定性
- **自訂分類與帳戶圖示**: 完整的圖示自訂能力
    - 支援自訂 Font Awesome 圖標輸入與關鍵字搜尋
    - 提供超過 2000 個 FontAwesome 內建圖示快速挑選
    - 即時圖標預覽功能
    - 可滾動的圖示選擇視窗，解決內容裁切問題
- **記錄編輯功能**: 詳細記錄頁面的完整編輯能力
    ...
    - 直觀的選中狀態視覺回饋
- **設定頁面增強**: 重新設計的設定介面
    - 新增「應用程式」區塊，包含「強制更新」、「分享 App」、「安裝為應用程式」等實用功能。
    - 清晰的功能分組和視覺層次，將資料管理、應用程式設定與版本資訊分開。
    - 版本資訊卡片顯示當前版本號。
...
系統支援四種外觀模式（`THEME_MODES`）：
- **跟隨系統 (`system`)**：預設模式，即時監聽作業系統 `prefers-color-scheme` 偏好自動切換深淺色。
- **淺色模式 (`light`)**：固定使用清新亮色 Wabi-sabi 風格。
- **深色模式 (`dark`)**：固定使用內建護眼深色主題。
- **自訂主題 (`custom`)**：套用從主題商店下載或手動匯入的第三方主題包。
...
- **小鍵盤無法最小化？** 確保在記帳頁面，點擊備註列旁的鍵盤圖示按鈕（⌨️）即可切換鍵盤顯示。
- **記錄編輯無法修改分類？** 在編輯模態框中先選擇類型（收入/支出），再選擇對應的分類。
- **找不到統計頁面？** 統計功能有專屬的底部導覽頁籤（📊 統計），直接點擊即可進入詳細分析。
...
1. **匯出資料**：設定頁面 → 資料管理 → 匯出資料
2. **匯入資料**：設定頁面 → 資料管理 → 匯入資料 → 選擇檔案
3. **檢查更新**：設定頁面 → 關於 → 檢查更新
4. **查看版本日誌**：設定頁面 → 關於 → 更新日誌
5. **應用程式管理**: 設定頁面 → 應用程式 → 點擊「強制更新」、「分享 App」或「安裝為應用程式」
...
5. **手動更新**：使用者可至「設定」頁面點擊「強制更新」來手動清除快取並重載。
...
- **單一執行實體 (Launch Handler)**：限制 PWA 單一執行視窗，點選圖示自動聚焦於已啟動的實體，防範多視窗寫入衝突。
- **Web 分享接收器 (Share Target)**：接收來自外部 App 的文字（例如銀行刷卡簡訊），自動以正則表達式剖析消費金額並帶入記帳備註，秒級記帳。
- **Edge 桌面小工具 (Widgets)**：整合 Windows 11 Widget Board。使用微軟 Adaptive Cards 定義 UI，背景呼叫 IndexedDB 計算今日收支事實，提供快速記帳與手動刷新。
...
<a href="https://github.com/ADT109119/jijun/graphs/contributors" target="_blank">
  <img src="https://contrib.rocks/image?repo=ADT109119/jijun"/>
</a>

...
- **AI 離線語音記帳助手**: 整合端側 58M LLM (WASM/wllama@3.5.1) 進行免連網語意 Tool Calling 解析，支援真實 GGUF 模型下載與 OPFS 離線快取，輸入口述中文自動擷取金額、收支類型、動態分類、帳戶與日期，並搭配高質感霧面玻璃、圓角與 Gemini 彩虹霓虹動態光罩，並提供離線規則引擎作為模型未就緒時的優雅降級機制。

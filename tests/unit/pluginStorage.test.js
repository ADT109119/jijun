...
describe('PluginStorage', () => {
    ...
    describe('constructor', () => {
        it('沒有 pluginId 拋出錯誤', () => {
            expect(() => new PluginStorage(null, mockDataService)).toThrow(
                'requires a pluginId'
            )
            expect(() => new PluginStorage('', mockDataService)).toThrow(
                'requires a pluginId'
            )
        })

        it('沒有 dataService 拋出錯誤', () => {
            expect(() => new PluginStorage('test-plugin')).toThrow(
                'requires a DataService instance'
            )
        })
    ...
    describe('setItem / getItem', () => {
        ...
        it('getItem 不會受到 prototype 污染', () => {
            ...
            expect({}.hasOwnProperty.call(storage.cache, 'constructor')).toBe(
                false
            )
        })
    })
    ...
    describe('init (migration)', () => {
        ...
        it('從 localStorage 遷移資料到 cache', async () => {
            ...
            localStorage.setItem(
                'plugin_test-plugin_migratedKey',
                'migratedValue'
            )

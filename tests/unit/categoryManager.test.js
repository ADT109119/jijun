...
describe('CategoryManager', () => {
    ...
    describe('addCustomCategory', () => {
        ...
        it('重複 ID 時回傳 false', async () => {
            ...
            const result = await cm.addCustomCategory('expense', {
                id: 'existing',
                name: '重名',
                icon: 'fas fa-ex2',
                color: 'bg-red-400',
    ...
    describe('removeCustomCategory', () => {
        ...
        it('移除不存在的自訂分類回傳 false', async () => {
            ...
            const result = await cm.removeCustomCategory(
                'expense',
                'nonexistent'
            )
    ...
    describe('updateCustomCategory', () => {
        ...
        it('更新不存在的自訂分類回傳 false', async () => {
            ...
            const result = await cm.updateCustomCategory('expense', {
                id: 'fake',
                name: 'Fake',
                icon: 'fas fa-fake',
                color: 'bg-red-400',
    ...
    describe('getCategoryById', () => {
        ...
        it('找不到回傳 null', () => {
            ...
            const result = cm.getCategoryById(
                'expense',
    ...
    describe('saveCategorySettings', () => {
        it('儲存 order 和 hidden', async () => {
            ...
            expect(result).toBe(true)
            expect(mockDS.saveCategorySetting).toHaveBeenCalledWith(
                'category_order',
                cm.categoryOrder
            )
            expect(mockDS.saveCategorySetting).toHaveBeenCalledWith(
    ...
    describe('closeManageCategoriesModal', () => {
        ...
        it('移除已建立的 modal', () => {
            document.body.innerHTML =
                '<div id="manage-categories-modal">test</div>'
            cm.closeManageCategoriesModal()
            expect(
                document.getElementById('manage-categories-modal')
            ).toBeNull()
        })
    })
    ...
    describe('getAllCategories edge cases', () => {
        it('order 中有不存在於任何分類的 ID 不會出錯', () => {
            cm.customCategories = { expense: [], income: [] }
            cm.categoryOrder = {
                expense: ['food', 'nonexistent_order_id'],
                income: [],
            }
            cm.hiddenCategories = { expense: [], income: [] }

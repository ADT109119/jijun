...
vi.mock('../../src/js/utils.js', async (importOriginal) => {
    ...
    return {
        ...
        getDateRange: vi.fn((period) => {
            if (period === 'month') return { startDate: '2026-06-01', endDate: '2026-06-30' };
            if (period === 'week') return { startDate: '2026-06-28', endDate: '2026-07-04' };
            if (period === 'today') return { startDate: '2026-06-30', endDate: '2026-06-30' };
            if (period === 'last7days') return { startDate: '2026-06-24', endDate: '2026-06-30' };
            if (period === 'year') return { startDate: '2026-01-01', endDate: '2026-12-31' };
...
function createMockDataService() {
    ...
    return {
        activeLedgerId: 1,
        getSetting: vi.fn(async (key) => settings[key] ? { key, value: settings[key] } : null),
        saveSetting: vi.fn(async ({ key, value }) => { settings[key] = value; return true; }),
...
describe('RecordsListManager - 明細預設時間範圍', () => {
    ...
    it('預設無任何設定與 Session 快取時，預設為本月', async () => {
        ...
        expect(container.querySelector('.period-btn[data-period="month"]').classList.contains('bg-wabi-surface')).toBe(true);
    });
    ...
    it('設定預設時間為「本週」時，載入本週時間區間並高亮週按鈕', async () => {
        ...
        expect(container.querySelector('.period-btn[data-period="week"]').classList.contains('bg-wabi-surface')).toBe(true);
    });
    ...
    it('設定預設時間為「今天」時，載入今天時間並高亮自訂按鈕', async () => {
        ...
        expect(container.querySelector('.period-btn[data-period="custom"]').classList.contains('bg-wabi-surface')).toBe(true);
    });
    ...
    it('設定預設時間為「上次時間範圍」時，載入上次時間範圍紀錄', async () => {
        ...
        await dataService.saveSetting({ key: 'lastRecordsPeriodState', value: { period: 'custom', customStartDate: '2026-06-10', customEndDate: '2026-06-20' } });
        await manager.init();
    ...
    it('優先讀取 session 暫存的過濾條件，而非預設設定', async () => {
        sessionStorage.setItem('jijun_records_filters', JSON.stringify({
            period: 'week', type: 'expense', categories: [], accounts: [],
            customStartDate: '2026-06-28', customEndDate: '2026-07-04', searchQuery: '測試',
        }));
...
describe('RecordsListManager - 轉帳抵消與摘要計算', () => {
    ...
    beforeEach(() => {
        ...
        dataService = createMockDataService();
        categoryManager = {
            getCategoryById: vi.fn().mockReturnValue({ name: '伙食', icon: 'fa-solid fa-utensils', color: '#f97316' }),
        };
        manager = new RecordsListManager(dataService, categoryManager, container);
    });
    ...
    it('轉帳配對成功時從摘要中排除', async () => {
        const transfers = [
            { id: 1, type: 'expense', category: 'transfer', amount: 500, date: '2026-06-15', description: '', debtId: null, amortizationId: null },
            { id: 2, type: 'income', category: 'transfer', amount: 500, date: '2026-06-15', description: '', debtId: null, amortizationId: null },
        ];
    ...
    it('轉帳配對失敗時保留在摘要中', async () => {
        const transfers = [
            { id: 1, type: 'expense', category: 'transfer', amount: 500, date: '2026-06-15', description: '', debtId: null, amortizationId: null },
            { id: 2, type: 'income', category: 'transfer', amount: 300, date: '2026-06-15', description: '', debtId: null, amortizationId: null },
        ];
        dataService.getRecords.mockResolvedValueOnce(transfers);
        await manager.init();
        expect(container.querySelector('#total-expense').textContent).toBe('$500');
        expect(container.querySelector('#total-income').textContent).toBe('$300');
    });

    it('普通紀錄正常計算摘要', async () => {
        const records = [
            { id: 1, type: 'expense', category: 'food', amount: 200, date: '2026-06-15', description: '午餐', debtId: null, amortizationId: null },
            { id: 2, type: 'income', category: 'salary', amount: 5000, date: '2026-06-01', description: '薪水', debtId: null, amortizationId: null },
        ];
        dataService.getRecords.mockResolvedValueOnce(records);
        await manager.init();
        expect(container.querySelector('#total-expense').textContent).toBe('$200');
        expect(container.querySelector('#total-income').textContent).toBe('$5,000');
    });
});
...
describe('RecordsListManager - 搜尋與類型過濾', () => {
    ...
    it('搜尋描述關鍵字會過濾紀錄', async () => {
        const records = [
            { id: 1, type: 'expense', category: 'food', amount: 200, date: '2026-06-15', description: '午餐便當', debtId: null, amortizationId: null },
            { id: 2, type: 'expense', category: 'food', amount: 300, date: '2026-06-16', description: '晚餐燒烤', debtId: null, amortizationId: null },
        ];
        dataService.getRecords.mockResolvedValueOnce(records);
        await manager.init();
    ...
    it('類型過濾為 expense 時只顯示支出', async () => {
        const records = [
            { id: 1, type: 'expense', category: 'food', amount: 200, date: '2026-06-15', description: '', debtId: null, amortizationId: null },
            { id: 2, type: 'income', category: 'salary', amount: 5000, date: '2026-06-01', description: '', debtId: null, amortizationId: null },
        ];
        dataService.getRecords.mockResolvedValueOnce(records);
        await manager.init();
...
describe('RecordsListManager - 標題顯示邏輯', () => {
    ...
    it('顯示某月初到月底時格式為「YYYY年MM月」', () => {
        ...
        expect(container.querySelector('#records-header-title').textContent).toBe('2026年6月');
    });
    ...
    it('同月非整月時顯示日期範圍', () => {
        ...
        manager.updateHeaderTitle();
        expect(container.querySelector('#records-header-title').textContent).toBe('2026年6月10號 ~ 20號');
    });
    ...
    it('跨月時顯示完整月份範圍', () => {
        ...
        manager.updateHeaderTitle();
        expect(container.querySelector('#records-header-title').textContent).toBe('2026年6月25號 ~ 7月5號');
    });
    ...
    it('跨年時顯示年份資訊', () => {
        ...
        manager.updateHeaderTitle();
        expect(container.querySelector('#records-header-title').textContent).toBe('2025年12月25號 ~ 2026年1月5號');
    });
});
...
describe('RecordsListManager - Session 過濾器管理', () => {
    ...
    it('_loadSessionFilters 回傳 Set 物件', () => {
        sessionStorage.setItem('jijun_records_filters', JSON.stringify({
            period: 'week', type: 'all', categories: ['food'], accounts: [], customStartDate: null, customEndDate: null, searchQuery: '',
        }));
...
describe('RecordsListManager - 欠款顯示邏輯', () => {
    ...
    it('未結清欠款顯示待還款狀態', async () => {
        const records = [
            { id: 1, type: 'expense', category: 'food', amount: 500, date: '2026-06-15', description: '', debtId: 1, amortizationId: null },
        ];
        dataService.getRecords.mockResolvedValueOnce(records);
        dataService.getDebt.mockResolvedValueOnce({ id: 1, type: 'receivable', settled: false, originalAmount: 200 });
        await manager.init();
        expect(container.querySelector('#records-list-container').textContent).toContain('待還款');
    });
    ...
    it('已結清欠款顯示已還清狀態', async () => {
        ...
        dataService.getRecords.mockResolvedValueOnce(records);
        dataService.getDebt.mockResolvedValueOnce({ id: 1, type: 'receivable', settled: true, originalAmount: 200 });
        await manager.init();
        expect(container.querySelector('#records-list-container').textContent).toContain('已還清');
    });

    it('欠款回收和償還類別不計入摘要', async () => {
        const records = [
            { id: 1, type: 'expense', category: 'debt_repayment', amount: 1000, date: '2026-06-15', description: '', debtId: null, amortizationId: null },
            { id: 2, type: 'income', category: 'debt_collection', amount: 2000, date: '2026-06-16', description: '', debtId: null, amortizationId: null },
            { id: 3, type: 'expense', category: 'food', amount: 200, date: '2026-06-15', description: '', debtId: null, amortizationId: null },
        ];
        dataService.getRecords.mockResolvedValueOnce(records);
        await manager.init();
        expect(container.querySelector('#total-expense').textContent).toBe('$200');

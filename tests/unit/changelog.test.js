...
describe('ChangelogManager', () => {
    ...
    it('能正確獲取最新版本號與 Changelog 資料', () => {
        ...
        expect(versions[0].version).toBe('2.1.7.0')
        expect(versions[0].title).toContain('真實 wllama 端側 LLM 推論整合')
    })
    ...
    it('當軟體版本升級 (lastSeenVersion !== currentVersion) 時應自動寫入 localStorage 並產生彈窗', () => {
        ...
        expect(localStorage.getItem('app-last-seen-version')).toBe('2.1.7.0')
        const modal = document.getElementById('update-changelog-modal')
        expect(modal).not.toBeNull()
        expect(modal.innerHTML).toContain('v2.1.7.0 登場！')
    })
    ...
    it('手動觸發 showUpdateChangelogModal 可開啟最新版本亮點 Modal', () => {
        ...
        const modal = document.getElementById('update-changelog-modal')
        expect(modal).not.toBeNull()
        expect(modal.querySelector('h3').textContent).toContain('v2.1.7.0 登場！')

...
describe('ComparisonReport.getLastYearPeriods', () => {
    it('將月份期間減一年', () => {
        const result = ComparisonReport.getLastYearPeriods([
            '2026-05',
            '2026-06',
        ])
    ...
    it('單一期間正常回傳', () => {
        expect(ComparisonReport.getLastYearPeriods(['2026-12'])).toEqual([
            '2025-12',
        ])
    })
})
...
describe('ComparisonReport.calculatePercentageBreakdown', () => {
    ...
    it('計算正確的百分比', () => {
        const result = ComparisonReport.calculatePercentageBreakdown(
            periodData,
            categoryComparisons
        )
...
describe('ComparisonReport.calculateDailyAverages', () => {
    it('計算正確的日均支出', () => {
        ...
        const result = ComparisonReport.calculateDailyAverages(
            periodData,
            'month',
            labels
        )
    ...
    it('空陣列回傳空陣列', () => {
        expect(
            ComparisonReport.calculateDailyAverages([], 'month', [])
        ).toEqual([])
    })
})
...
describe('ComparisonReport.calculateComparison', () => {
    ...
    it('當 typeFilter=expense 時，只應包含支出紀錄', async () => {
        ...
        const comp = new ComparisonReport(mockDS, null)
        const result = await comp.calculateComparison(
            'month',
            ['2026-05', '2026-06'],
            { typeFilter: 'expense' }
        )

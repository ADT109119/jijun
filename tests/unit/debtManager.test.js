...
function createMockDataService() {
  ...
  return {
    ...
    addContact: vi.fn(async contact => {
      const id =
        contacts.length > 0 ? Math.max(...contacts.map(c => c.id)) + 1 : 50
    ...
    addRecord: vi.fn(async record => {
      const id =
        records.length > 0 ? Math.max(...records.map(r => r.id)) + 1 : 200
...
describe('DebtManager - 核心業務邏輯', () => {
  describe('金額計算 (remainingAmount 回退邏輯)', () => {
    it('有 remainingAmount 時使用 remainingAmount', () => {
      const debts = [
        { type: 'receivable', remainingAmount: 5000, originalAmount: 10000 },
      ]
